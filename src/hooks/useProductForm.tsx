...
export const useProductForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "There was an error saving the product",
        variant: "destructive",
      });
    } finally {
...
// // Update the handleSubmit function to upload the file first
// const uploadImage = async (file: File): Promise<string | null> => {
//   try {
//     const fileExt = file.name.split('.').pop();
//     const fileName = `${Math.random().toString(36).substring(2)}-${Date.now()}.${fileExt}`;
...
//     const { data, error } = await supabase.storage
//       .from('product-images') // Make sure this bucket exists in Supabase Storage
//       .upload(filePath, file);
